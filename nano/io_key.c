  ...
  if (!(*raw & 0x80))
  ...
  else if ((*raw & 0xF0) == 0xE0)
    {
      if ((raw[1] & 0300) == 0200 && (raw[2] & 0300) == 0200)
        {
          glyph =
            ((raw[0] & 0xF) << 12) + ((raw[1] & 0x3F) << 6) + (raw[2] & 0x3F);
          raw += 3;
        }
      else
        return 0;
  ...
  else if ((*raw & 0xF8) == 0xF0)
    {
      if ((raw[1] & 0300) == 0200 && (raw[2] & 0300) == 0200
          && (raw[3] & 0300) == 0200)
        {
          glyph = ((raw[0] & 0x7) << 18) + ((raw[1] & 0x3F) << 12)
                  + ((raw[2] & 0x3F) << 6) + (raw[3] & 0x3F);
          raw += 4;
        }
      else
        return 0;
...
static void keyin_event (io_stream_t *stream, int events)
{
  ...
  ssize_t rn = read (stream->fd, data_end,
                     (size_t) (sizeof ks->buffer + buffer - data_end - 1));
  if (rn < 0)
    return;
...
int io_key_on (seq_handler_t *onseq, void *data)
{
  if (!io_key_stream)
    {
      auto key_stream_t *ks = io_key_stream =
        (key_stream_t *) calloc (1, sizeof (key_stream_t));
      io_stream_init (&io_key_stream->stream, STDIN_FILENO, POLLIN,
                      &io_key_ops);
