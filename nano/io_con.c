...
static void stdout_nonblock_set (int yes)
{
  ...
  fcntl (STDOUT_FILENO, F_SETFL,
         yes ? flags & ~O_NONBLOCK : flags | O_NONBLOCK);
}
...
void io_con_init ()
{
  conout_stream_t *co = io_con_stream =
    (conout_stream_t *) calloc (1, sizeof (conout_stream_t));
  ...
  if (go <= po && tail_size < size)
    {
      ...
      memcpy (buff, data + tail_size,
              po = (unsigned) (size - (unsigned) tail_size));
    }
  else
    {
          ...
          case 'd':
            {
              ...
              int arg = va_arg (ap, int), minus = arg < 0;
              if (minus)
                arg = -arg;
              do
                {
