...
{
  con_buf.end =
    (con_buf.start = con_buf.ptr = (char *) malloc (CONSOLE_BUFFER_SIZE))
    + CONSOLE_BUFFER_SIZE;
  con_buf.start[0] = 0;
}
...
void con_flush ()
{
  safe_write (STDOUT_FILENO, con_buf.start,
              (size_t) (con_buf.ptr - con_buf.start));
...
static void con_buf_extend (size_t req_size)
{
  ...
  size_t buf_size = (unsigned) (con_buf.end - con_buf.start),
         length = (unsigned) (con_buf.ptr - con_buf.start), new_size = buf_size;
  do
    {
      new_size *= 2;
    }
