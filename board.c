...
/* -------------------------------------------------------------------------- */
void
board_figure_set(board_t* self, int id)
{
  self->figure.id = id;
  self->figure.angle = 0;
...
static int
board_figure_submove(board_t* self, int dx, int dy, int dang)
{
  self->figure.x += dx;
  self->figure.y += dy;
  ...
  if (figure_test(&self->figure, &self->field)) {
    if (!dx && !dy && dang &&
        self->figure.y < 0) { // patch for "line" fig. rotation at top position
      self->figure.y = 0;
      if (!figure_test(&self->figure, &self->field))
        return 0;
    } else
...
/* -------------------------------------------------------------------------- */
int
board_figure_move(board_t* self, int dx, int dy, int dang)
{
...
/* -------------------------------------------------------------------------- */
int
board_figure_drop(board_t* self)
{
  figure_draw(&self->figure, &self->field, 0);
  int c = 0;
