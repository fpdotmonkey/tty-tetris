...
/* -------------------------------------------------------------------------- */
void board_init (board_t *self, int left, int top, unsigned int width,
                 unsigned int height, char const *space)
{
...
static int board_figure_submove (board_t *self, int dx, int dy, int dang)
{
  ...
  if (figure_test (&self->figure, &self->field))
    {
      if (!dx && !dy && dang && self->figure.y < 0)
        {  // patch for "line" fig. rotation at top position
          self->figure.y = 0;
          if (!figure_test (&self->figure, &self->field))
            return 0;
        }
