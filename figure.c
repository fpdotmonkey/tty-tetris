...
/* ------------------------------------------------------------------------ */
static unsigned int
get_figure(int fig, int angle)
{
  static const unsigned short c_figures[] = {
    0x0156, 0x1458, 0x0156, 0x1458, // Z
    0x1245, 0x0459, 0x1245, 0x0459, // S
    0x2456, 0x159A, 0x4568, 0x0159, // L
    0x0456, 0x1259, 0x456a, 0x1589, // J
    0x1456, 0x1569, 0x4569, 0x1459, // T
    0x4567, 0x159d, 0x4567, 0x159d, // I
...
/* ------------------------------------------------------------------------ */
void
figure_init(figure_t* self, int id)
{
...
/* ------------------------------------------------------------------------ */
void
figure_draw(figure_t* self, field_t* field, int show)
{
  int id = self->id;
  if (id < 0)
    return;
  ...
  for (int i = 0; i < FIGURE_LENGTH; ++i, fig >>= 4)
    field_set_cell(
      field, x + (signed)(fig & 3), y + (signed)((fig >> 2) & 3), fill);
}

/* ------------------------------------------------------------------------ */
int
figure_test(figure_t* self, field_t* field)
{
  int id = self->id;
  if (id < 0)
    return 1;
  ...
  for (int i = 0; i < FIGURE_LENGTH; ++i, fig >>= 4)
    if (field_get_cell(
          field, x + (signed)(fig & 3), y + (signed)((fig >> 2) & 3)))
      return 1;
  return 0;
}
